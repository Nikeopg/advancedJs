    ...
    */
    class Cache {
    	...
    	isDetached(target, object) {
    		return !Object.is(target, path$1.get(object, this.getPath(target)));
    	}
    ...
    const onChange = (object, onChange, options = {}) => {
    	...
    	const validate = (target, property, value, previous, applyData) => !hasOnValidate
    		...
    		|| options.onValidate(path$1.concat(cache.getPath(target), property), value, previous, applyData) === true;
    	...
    	const handleChange = (changePath, property, value, previous, applyData) => {
    		if (smartClone.isCloning) {
    			...
    			onChange(path$1.concat(changePath, property), value, previous, applyData);
    		}
    	};
    	...
    	const prepareValue = (value, target, property, basePath) => {
    		...
    		In this case we return the proxy object with the shortest path.
      		*/
    		const childPath = path$1.concat(basePath, property);
    	...
    	const handler = {
    		...
    		apply(target, thisArg, argumentsList) {
    			...
    			if (
    				...
    				let applyPath = path$1.initial(cache.getPath(target));
    				...
    				if (SmartClone.isHandledType(result) && isHandledMethod) {
    					if (thisArg instanceof Map && target.name === 'get') {
    						applyPath = path$1.concat(applyPath, argumentsList[0]);
    					}
    				...
    				if (isChanged) {
    					...
    					const changePath = smartClone.isCloning
    						? path$1.initial(applyPath)
    						: applyPath;
    					const property = smartClone.isCloning
    						? path$1.last(applyPath)
    						: '';

    					if (validate(path$1.get(object, changePath), property, thisProxyTarget, previous, applyData)) {
